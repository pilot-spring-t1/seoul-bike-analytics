...
public class NotificationServiceImpl implements NotificationService {
    ...
    public void createNotification(NotificationDto notificationDto) {
        ...
        if (Long.valueOf(-1L).equals(notificationDto.getMemberId())) {
            ...
            for (Member member : allMembers) {
                // [핵심] 발신자(senderId)와 수신자(memberId)가 같으면 알림을 생성하지 않음
                ...
                dto.setSenderId(notificationDto.getSenderId()); // 발신자 정보 전파
            ...
            if (!notificationList.isEmpty()) {
                // 2. DB 일괄 저장 (SENDER_ID 포함)
                notificationMapper.insertNotifications(notificationList);
                log.info("=> [DB] {}건의 알림 일괄 저장 완료 (발신자 제외됨)", notificationList.size());
            ...
            log.info("=> [DB] 단건 알림 저장 완료 (Sender: {})", notificationDto.getSenderId());
    ...
    @Override
    public void removeNotification(Long notiId, Long memberId) {
